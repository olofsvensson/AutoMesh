...
def autoMesh(snapshotDir, workflowWorkingDir, autoMeshWorkingDir, loopMaxWidth=300, loopMinWidth=150, prefix="snapshot", debug=False, findLargestMesh=False):
    ...
    for omega in [0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330]:
        ...
        if debug:
            plot_img(differenceImage, plotPath=os.path.join(autoMeshWorkingDir, "differenceImage_%03d.png" % omega))

#        print numpyImage
        filteredImage = filterDifferenceImage(differenceImage)
        if debug:
            plot_img(filteredImage, plotPath=os.path.join(autoMeshWorkingDir, "filteredImage_%03d.png" % omega))
    ...
    (angleMinThickness, x1Pixels, y1Pixels, dxPixels, dyPixels, deltaPhiz, stdPhiz) = findOptimalMesh(dictLoop, snapshotDir, nx, ny, autoMeshWorkingDir, debug=debug,
                                                                                                      loopMaxWidth=loopMaxWidth, loopMinWidth=loopMinWidth,
                                                                                                      findLargestMesh=findLargestMesh)
    imagePath = None
    if angleMinThickness is not None:
        imagePath = os.path.join(snapshotDir, "%s_%03d.png" % (prefix, angleMinThickness))
    return (angleMinThickness, x1Pixels, y1Pixels, dxPixels, dyPixels, deltaPhiz, stdPhiz, imagePath, areTheSameImage)
...
def filterDifferenceImage(differenceImage, thresholdValue=30):
    ...
    filteredImage = scipy.ndimage.morphology.binary_erosion(binaryImage).astype(binaryImage.dtype)
    filteredImage = scipy.ndimage.morphology.binary_erosion(filteredImage).astype(binaryImage.dtype)
    filteredImage = scipy.ndimage.morphology.binary_dilation(filteredImage).astype(binaryImage.dtype)
    filteredImage = scipy.ndimage.morphology.binary_dilation(filteredImage).astype(binaryImage.dtype)
...
def findOptimalMesh(dictLoop, snapshotDir, nx, ny, autoMeshWorkingDir, loopMaxWidth=300, loopMinWidth=150,
    ...
    for omega in [0, 30, 60, 90, 120, 150]:
        ...
        if True:
            ...
            pylab.savefig(os.path.join(autoMeshWorkingDir, "shapePlot_%03d_%03d.png" % (omega, omega + 180)))
            pylab.close()
        phiz = None
        if (arrayUpper1.shape == arrayLower2.shape) and (arrayUpper2.shape == arrayLower1.shape):
        ...
        if phiz is not None:
            if debug:
                pylab.plot(phiz, "+")
                pylab.savefig(os.path.join(autoMeshWorkingDir, "phiz_%03d_%03d.png" % (omega, omega + 180)))
                pylab.close()
    ...
    for omega in [0, 30, 60, 90, 120, 150, 180, 210, 270, 300, 330]:
        ...
        if debug:
            ...
            strPhizPath = os.path.join(autoMeshWorkingDir, "thickness_%s.png" % strOmega)
            pylab.savefig(strPhizPath)
            pylab.close()
...
#                tmpMin = 75
    ...
    meshXmin -= 50
    logging.debug("Max thickness = %r pxiels at omega %r" % (maxThickness, angleMaxThickness))
    logging.debug("Mesh: xMin=%r, xMax=%r, yMin=%r, yMax = %r" % (meshXmin, meshXmax, maxThicknessMeshYmin, maxThicknessMeshYmax))
    logging.debug("Min thickness = %r pixels at omega %r" % (minThickness, angleMinThickness))
    logging.debug("Mesh: xMin=%r, xMax=%r, yMin=%r, yMax = %r" % (meshXmin, meshXmax, minThicknessMeshYmin, minThicknessMeshYmax))
    if meshXmin < 10:
        meshXmin = 10
    ...
    logging.debug("angle={0}, x1Pixels={1}, y1Pixels={2}, dxPixels={3}, dyPixels={4}, deltaPhiz={5}, stdPhiz={6}".format(angle, x1Pixels, y1Pixels, dxPixels, dyPixels, deltaPhiz, stdPhiz))
